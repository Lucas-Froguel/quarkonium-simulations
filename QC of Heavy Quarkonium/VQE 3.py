...
def build_callback(ansatz, hamiltonian, estimator, callback_dict):
    ...
    def callback(current_vector):
        ...
        estimate = estimator.run(ansatz, hamiltonian, parameter_values=current_vector).result()
        callback_dict["mean_energies"].append(
            estimate.values[0]
        )
        callback_dict["var_energies"].append(
            estimate.metadata[0]["variance"]
        )
...
save_pickle(res)
save_pickle(callback_dict, filename="callback_dict.pickle")
print(res)

fig, ax = plt.subplots()
ax.errorbar(range(callback_dict["iters"]), callback_dict["mean_energies"], yerr=callback_dict["var_energies"])
